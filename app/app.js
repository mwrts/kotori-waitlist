...
function extractFurigana(surface, reading) {
    // Support [Kanji](furigana) syntax
    const markupRegex = /\[([^\]]+)\]\(([^)]+)\)/g;
    if (markupRegex.test(surface)) {
        const parts = [];
        let lastIdx = 0;
        surface.replace(markupRegex, (match, kanji, furigana, offset) => {
            if (offset > lastIdx) parts.push({ text: surface.substring(lastIdx, offset), rt: '' });
            parts.push({ text: kanji, rt: furigana });
            lastIdx = offset + match.length;
        });
        if (lastIdx < surface.length) parts.push({ text: surface.substring(lastIdx), rt: '' });
        return parts;
    }
...
function updateSidebarInfo(block, index, def = null) {
    ...
    if (existingWord) {
        ...
        contextEl.classList.add('opacity-0');
        contextEl.innerText = '-';
    }
...
function renderReader() {
    const doc = appState.docs.find(d => d.id === appState.activeDocId);
    document.getElementById('reader-title').innerHTML = (doc ? doc.title : 'no document') + ' <span class="text-[10px] opacity-20 ml-2">v19</span>';
